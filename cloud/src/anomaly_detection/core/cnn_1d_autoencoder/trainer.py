...
class Trainer:
    ...
    def train_epoch(self, train_loader, epoch_idx: Optional[int] = None) -> float:
        ...
        for batch in train_loader:
            ...
            self.train_step((sequences, targets))
            clean_total += self._compute_eval_loss_batch(sequences, targets)
            count += 1

        reported_loss = clean_total / max(count, 1)
        self.metrics["train_losses"].append(reported_loss)
        return reported_loss

    def _compute_eval_loss_batch(self, sequences, targets) -> float:
        """计算当前批次在推理模式下的损失，用于展示。"""
        self.model.set_train(False)
        eval_loss = self.compute_loss((sequences, targets))
        self.model.set_train(True)
        return eval_loss
