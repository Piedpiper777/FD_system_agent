...
输出到 collected/AnomalyDetection 目录，文件名体现数据类型
"""
...
def generate_anomaly_detection_data(
    ...
    for i in range(num_normal):
        for rpm in rpm_values:
            ...
            filename = f"anomaly_detection_normal_rpm{rpm:.0f}_{i+1:03d}.csv"
            filepath = output_dir / filename
            df.to_csv(filepath, index=False)
            
            file_count += 1
            print(f"✓ 生成正常数据: {filename}")
    
    # 生成异常数据（使用故障数据）
    # 异常检测不需要区分故障类型，所有故障都作为"异常"
    # 使用内圈和外圈故障作为异常样本，增加数据多样性
    fault_types = ['inner_ring', 'outer_ring']
    for i in range(num_anomaly):
        for rpm in rpm_values:
            fault_type = fault_types[i % len(fault_types)]
            data = generator.generate_fault_data(duration, rpm, fault_type, fault_severity=3.0)
            df = generator.create_dataframe(data)
            
            # 文件名中保留故障类型信息（便于数据管理），但都视为"异常"
            filename = f"anomaly_detection_anomaly_{fault_type}_rpm{rpm:.0f}_{i+1:03d}.csv"
            filepath = output_dir / filename
            df.to_csv(filepath, index=False)
            
            file_count += 1
            print(f"✓ 生成异常数据: {filename}")
    
    print(f"\n异常检测数据生成完成，共 {file_count} 个文件")


def main():
    # ==================== 配置参数 ====================
    # 输出目录（相对于项目根目录）
    OUTPUT_DIR = 'edge/data/collected/AnomalyDetection'
    ...
    print(f"正常样本数: {NUM_NORMAL}")
    print(f"异常样本数: {NUM_ANOMALY}")
    ...
    print("- 异常数据：包含内圈故障和外圈故障的异常数据")
    print("\n数据特征：")
    print("- 不同转速：影响振动频率和振幅")
    print("- 内圈故障：高频冲击（5.43倍转频），X轴振幅显著增大")
    print("- 外圈故障：高频冲击（3.57倍转频），振幅增大相对较小")
    print("\n文件名格式：")
    print("- 正常：anomaly_detection_normal_rpm{转速}_{序号}.csv")
    print("- 异常：anomaly_detection_anomaly_{故障类型}_rpm{转速}_{序号}.csv")
